...
ACCEPTABLE_RESOURCE = (
    "Acceptable resources URI: repositories (github.com/:user/:repo), "
    "folders (github.com/:user/:repo/tree/:branch/:path), "
    "files (github.com/:user/:repo/blob/:branch/:path) "
    "and raw files (raw.githubusercontent.com/:user/:repo/:branch/:path)"
)
    ...
    def get_all_mementos(self, uri):
        ...
        # Resource is a raw file
            ...
            def make_pair(commit):
                ...
                uri_m = '%s%s%s/%s%s' % (protocol, base,
                                         user, repo, memento_path)
                return (uri_m, commit['commit']['committer']['date'])
            mapper = make_pair
