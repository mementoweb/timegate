...
__author__ = 'Harihar Shankar, Yorick Chollet'
...
class OrainHandler(Handler):
    ...
    def get_memento(self, req_uri, accept_datetime):
        ...
        try:
            ...
            logging.debug("Orain handler: API found: %s, page title parsed to: %s " % (api_base_uri, title) )
            if not title:
            ...
            logging.error("OrainHandler: querying and parsing page for title/api %s. handler will return empty response" % e)
            return None
